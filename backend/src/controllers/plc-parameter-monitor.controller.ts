...
export const getPLCTemperature = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Read current values from DB which already sync real time
    const variablesWithPLCValues = await plcVariableService.getVariables({type: 'May_tinh_Nhiet_Muc'})
...
export const getPLCElectricity = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Read current values from DB which already sync real time
    const variablesWithPLCValues = await plcVariableService.getVariables({type: 'May_tinh_Chinh_luu_R'})
